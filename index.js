...
app.post('/write', (req, res) => {
	if (authenticate(req, res)) {
		...
		if (data) {
			fetch("https://jsonstorage.net/api/items/38ceee94-8f71-4399-ab9a-4f51043baab8", {
				method: 'PUT',
				headers: {
			...
			// fs.writeFile('public/data/lives.json', data, err => {
			// 	if (err) return res.send(["Ocorreu um erro desconhecido ao salvar o arquivo"])

			// 	fs.writeFile(`public/data/backups/bkp-${Date.now()}.json`, data, err => {
			// 		if (err) return res.send(["Ocorreu um erro desconhecido ao salvar o arquivo"])
			// 		res.send([])
			// 	})
			// })
